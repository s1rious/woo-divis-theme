 ...
 */
class _WC_Widget_Layered_Nav extends WC_Widget {
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				if ($term->taxonomy == 'pa_color') {
					...
					$color_field = get_field('cs-color', $term);
					$term_html .= '<label class=\'cs-'.$term->name.'\' style="background-color:'.$color_field.'">';
			...
			echo '<li class="cs-item ' . ( $option_is_set ? 'woocommerce-widget-layered-nav-list__item--chosen chosen' : '' ) . '">';
